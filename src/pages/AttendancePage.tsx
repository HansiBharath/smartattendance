...
message: 'Mark your attendance before 11:00 AM',
time: '30 minutes ago',
priority: 'high'
},
];
          ...
          {[1, 2, 3, 4].map((step, index) => (  
            <div key={step} className="flex items-center">  
              <div className={cn(  
                ...
                getStepStatus(step) === 'active' && "bg-blue-500 border-blue-500 text-white",  
                getStepStatus(step) === 'pending' && "bg-gray-200 border-gray-300 text-gray-500"  
              )}>  
                {getStepStatus(step) === 'completed' ? <Check className="h-5 w-5" /> : step}  
              </div>  
              {index < 3 && (  
                <div className={cn(  
                  "w-24 h-0.5 mx-2 transition-all duration-300",  
      ...
      <div className="space-y-6">  
        ...
        {currentStep >= 1 && (  
          <Card className={cn(  
            ...
            currentStep === 1 ? "ring-2 ring-blue-500" : currentStep > 1 ? "opacity-75" : ""  
          )}>  
              ...
              {currentStep === 1 ? (  
                <div className="space-y-4">  
                  ...
                  <Button   
                    ...
                    disabled={!registerNo.trim()}  
                    className="w-full bg-teal-600 hover:bg-teal-700 text-white py-3"  
                  >  
                    Submit Register Number  
                  </Button>  
                </div>  
              ) : (  
        ...
        {/* Step 4: Confirmation */}  
        {currentStep >= 4 && (  
          <Card className="attendance-confirmation-card">  
            <CardContent className="p-8 text-center">  
              ...
              <h3 className="text-2xl font-bold text-green-600 mb-2">Attendance Marked Successfully!</h3>  
              <p className="text-gray-600 mb-6">Your attendance has been recorded with the following details:</p>  
                
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left bg-gray-50 p-4 rounded-lg">  
                <div className="flex items-center gap-2">  
                  <User className="h-4 w-4 text-gray-500" />  
                  <span className="text-sm"><strong>Register No:</strong> {registerNo}</span>  
                </div>  
                <div className="flex items-center gap-2">  
                  <Clock className="h-4 w-4 text-gray-500" />  
                  <span className="text-sm"><strong>Time:</strong> {new Date().toLocaleTimeString()}</span>  
                </div>  
                <div className="flex items-center gap-2">  
                  <Clock className="h-4 w-4 text-gray-500" />  
                  <span className="text-sm"><strong>Date:</strong> {new Date().toLocaleDateString()}</span>  
                </div>  
                <div className="flex items-center gap-2">  
                  <MapPin className="h-4 w-4 text-gray-500" />  
                  <span className="text-sm"><strong>Location:</strong> Verified</span>  
                </div>  
              </div>  
